...
There is no sign-up. Guests enter through one-time invite links (`/invite/<token>`); joining binds the browser via the `hb_session` cookie, whose token is passed as the `sessionToken` argument to every gated Convex function (`convex/lib/auth.ts`: `sessionQuery` / `sessionMutation` / `adminQuery` / `adminMutation`). Routes behind an invite live under the `_guest` pathless layout (`src/routes/_guest.tsx`), which puts `sessionToken` and `me` into route context — use `src/lib/guest.ts` (`useMe`, `useSessionMutation`). Admins are users with `isAdmin: true`: toggle on `/admin`, mint admin invites with `bun run invite --admin`, or as a last resort `bunx convex run users:setAdminInternal '{"userId":"...","isAdmin":true}'`.
